...
<Note>
  Prefer not to use the browser, or on a headless box like a CI runner? Create a personal access token in the dashboard (**Settings → Developer → Personal access tokens**) and use `nubo login --with-token`, or pipe it in with `nubo login --token-stdin`. See [API authentication](/api-reference/authentication) for more on tokens.
</Note>
