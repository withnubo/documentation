...
---
<Update label="July 17th, 2026" tags={["UI", "Dashboard", "Frames", "Bug-fixes"]}>

## Project logs, a refreshed homepage, and reliability fixes

<Frame>
  <img src="/images/release-notes/07-17-2026/project_logs.png" alt="logs page" />
</Frame>

This release brings a unified Project Logs page, a refreshed withnubo.com homepage, and a round of reliability fixes for the gateway and deployments.

### Project logs

Search across every Frame and Space in your project from one place, no more digging through individual build or runtime logs.

- Powerful filter bar lets you search by keyword, exact phrase, or regex.
- Combine filters with AND / OR / NOT, and narrow by space, frame, status, log level, or time range.
- Live-tail support so you can watch new log lines come in as they happen.
- Instantly find every error, warning, or specific issue across your whole project, no matter which Frame it came from.

### Homepage update

We refreshed the withnubo.com homepage with a new look, including an animated banner that shows off our different products, sections highlighting key features, and a live counter showing real deployments happening right now.

### Smaller things worth mentioning

- **Gateway reliability**: Fixed a background issue that could affect how traffic is routed. The system now clearly signals if something goes wrong instead of failing silently, and brief network hiccups no longer disrupt service.
- **Deployment reliability**: Fixed an issue where some deployments could get stuck due to resource limits, and a bug so existing deployments update correctly. Deployments now consistently use the right setup, and addresses stay stable even after redeploying.

Questions or feedback: `support@withnubo.com` or [Discord](https://discord.com/invite/4SM4faYqWQ).

</Update>

<Update label="July 10th, 2026" tags={["UI", "Dashboard", "Frames", "Bug-fixes"]}>

## A design overhaul, public profiles, and searchable build logs

![The redesigned Nubo interface](/images/release-notes/07-10-2026/design.png)

Nubo has a brand-new, optimized look. We've refreshed the entire interface while preserving all the functionality and features you rely on. Alongside the redesign: public profiles and organizations, an improved Frame overview with searchable build logs, a customizable navigation bar, and a round of smaller fixes.

### Public profiles and organizations

<Frame>
  <img src="/images/release-notes/07-10-2026/public profile.png" alt="Public profiles" />
</Frame>

We're introducing public profiles and organizations. Enable a public-facing profile from your Settings page, then toggle individual Frames to public visibility from that Frame's settings. Public Frames appear on your profile, which you can share via a link found in Settings. Our goal is to help you showcase your projects and share your work more easily with the world.
<Columns cols={2}> 
<Frame>
  <img src="/images/release-notes/07-10-2026/orgs.png" alt="organizations" />
</Frame>
<Frame>
  <img src="/images/release-notes/07-10-2026/profile.png" alt="Profile overview" />
</Frame>

</Columns>

### Improved Frame overview and build logs

We've changed the Frame management flow: rather than the sidebar widget, Frames now open in a pop-up window. We've also added new functionality to the Frame Logs tab. Logs now include a search bar above the log output, so you can search by keyword, timestamp, or date, and buttons below the search bar let you jump directly to specific keyword matches within the logs. We hope this makes troubleshooting faster and gives you clearer insight into what's changed.

### A new, customizable navigation bar
<Columns cols={2}> 
To help streamline your workflow, you can now edit and manage which widgets appear in your navigation bar, as well as the order in which they're stacked.
<Frame>
  <img src="/images/release-notes/07-10-2026/widget.gif" alt="Widget demo" />
</Frame>
</Columns>
<Columns cols={2}>
<Frame>
  <img src="/images/release-notes/07-10-2026/activity_widget.png" alt="Activity widget" className="rounded-lg" noZoom/>
</Frame>
<Frame>
  <img src="/images/release-notes/07-10-2026/shortcut_widget.png" alt="Recent widget" className="rounded-lg" noZoom/>
</Frame>
</Columns>
<Columns cols={2}>
<Frame>
  <img src="/images/release-notes/07-10-2026/recent_widget.png" alt="Recent widget" className="rounded-lg" noZoom/>
</Frame>
<Frame>
  <img src="/images/release-notes/07-10-2026/plan_widget.png" alt="Recent widget" className="rounded-lg" noZoom />
</Frame>
</Columns>

### Smaller things worth mentioning

- **Docs cleanup pass**: Fixed a bunch of small inaccuracies across the docs (plans, API versions, previews, build logs, and more), removed some broken links/images, and added new pages for MCP server, Teams, Organizations, Account security, Metrics, and Studio.
- **Frame resource migration, fixed**: A background fix that correctly applies the right CPU/memory limits to existing Frames based on your plan tier — no Frames were downgraded, and everything's now sized correctly.
- **Preview builds, more reliable**: Fixed an issue where some preview deployments could get stuck or fail to load, and made sure previews now build with the correct builder (Raypacks or Paketo) to match your Frame's setup.
...
<Update label="July 3rd, 2026" tags={["MCP", "Plans", "Bug-fixes"]}>

## New: Nubo MCP Server Support

Nubo now supports MCP, making it easy to connect your projects to AI assistants and other MCP-compatible clients. You can securely inspect, deploy, and manage your infrastructure using your existing Nubo account and permissions.

Every MCP request is authenticated with your Nubo account and sent through the existing API, so your permissions, rate limits, role-based access control, and audit logs continue to work exactly as they do today.

- Connect Nubo to any MCP-compatible client.
- Use a local MCP server or connect through Nubo's hosted endpoint.
- Authenticate with your existing Nubo account with no additional credentials required.
- Grant clients only the permissions they need with scoped access: read, deploy, and write.
- No separate backend or permission system; every request goes through Nubo's existing API.

Start using AI to inspect, deploy, and manage your Nubo projects while keeping the same security model you're already familiar with.

### Getting Started

There are two ways to get started with Nubo's MCP server: you can connect your MCP client, or run it locally instead.

**Connecting your client (Claude Code)**

If you're using Claude Code, you can connect your client by running:

```bash
claude mcp add --transport http nubo https://mcp.withnubo.com/mcp
```

**Running it locally**

You can also run it locally using the install command:
...
```
## New pricing: more plans, more flexibility

We've updated our pricing to give you more options depending on where you are and what you need. There's a new plan in the middle of the lineup, self-hosted agents get simple flat pricing, and a round of bug fixes rides along.

### Pick the plan that fits

Plans now start at \$9 a month and come with \$9 of usage credit, so you only pay for what you actually use — no surprise bills, no paying for idle resources.

- **Trial** — free for seven days, no card required.
- **Basic** — \$9 per seat, includes \$9 of usage credit.
- **Silver** — $29 per seat, adds more headroom, storage, and support.
- **Platinum** — $59 per seat, for teams that need the most Nubo has to offer.

Rather run it on your own hardware? **Self-hosted agents** are $5 per agent per month and let you deploy through Nubo on your own infrastructure. See [Plans](/plans) for the full breakdown.

### Bug fixes

- New Frames now use the correct builder on their first deploy.
- A Frame's public URL stays stable across redeploys — no more random URL changes.
- Preview deployments no longer time out due to server scheduling issues.
- Preview builds now respect the Frame's selected builder instead of defaulting to Paketo.
- Existing Frames now correctly reflect the resource limits for their plan tier.

Questions or feedback: `support@withnubo.com` or [Discord](https://discord.com/invite/4SM4faYqWQ).

</Update>

<Update label="June 26th, 2026" tags={["Deployments", "Raypacks"]}>

## nubo.toml, the one config file Raypacks reads

Raypacks works without any config for most apps. But when you need to step in and tell it something it can't figure out on its own, you can now drop a `nubo.toml` at the root of your repo and take control. Think of it as your single source of truth for how your app gets built.

- **Pick a specific binary** when you have more than one. Set the entry point and Raypacks builds from exactly there.
- **Don't need it? Don't add it.** Raypacks auto-detects most setups, so the file is completely optional.
- **Bad config gets caught before the build starts**, so you'll see a clear error in your logs instead of a confusing failure ten minutes in.
- **Every build shows a plan summary** of the final values and where each one came from, so there's no guessing what Raypacks decided.

Best of all, `nubo.toml` acts like a blueprint for your project: a source of truth you scaffold once, then stand up your entire stack in minutes.

### Smaller things worth mentioning

- The default builder is now Raypacks.
...
<Update label="June 19th, 2026" tags={["Deployments", "Analytics", "Support", "Raypacks", "Bug-fixes"]}>

## Deploy from a subdirectory, real request analytics, and a faster builder in beta

This release brings subdirectory deploys for monorepos, per-Frame request analytics straight from our edge, in-app support tickets, and a new daemonless builder in beta. Plus a round of bug fixes.

### Deploy from a subdirectory

Got a monorepo where your app lives in a subfolder instead of the root? Now you can set a **source directory** and Nubo builds from exactly that folder.

- Set it when you first create a Frame.
- Change it anytime in **Frame Settings**.
- Works on managed cloud and on your own agents.

### Per-Frame request analytics

You can now see real HTTP request traffic for each of your Frames, captured directly from Nubo's own edge, so what you're seeing is what's actually happening.

- View volume, latency, status codes, and egress per Frame.
- This isn't an estimate scraped from infrastructure metrics; it's real request traffic.

Knowing how your app performs under live traffic is the difference between reacting to problems and preventing them. More depth is on the way, but the core picture is live now.

### In-app support tickets

You can now open and view any support tickets you've opened, right from the dashboard.

### Beta: faster daemonless builds and layer cache

You can now replace your Paketo / Cloud Native Buildpacks build path with **Raypacks**, a daemonless OCI buildpack system focused on speed and cache efficiency. This feature is still in beta, so we're testing for and addressing bugs as they appear.

### Bug fixes

- Fixed a bug where you couldn't upgrade your account.
- Fixed a bug where the repo picker would leak another tenant's repos to GitHub collaborators.
- Fixed a bug where a Frame settings patch would overlay onto unrelated fields.
- Fixed a bug where an "Invalid request body" error would pop up when submitting a form without first expanding the "General" FAQ dropdown.
...
<Update label="June 1st, 2026" tags={["Teams", "Security", "Databases", "Metrics", "Frames", "CI/CD", "Previews", "Dashboard"]}>
...
Add a second factor so a borrowed GitHub session isn't enough to get into your account.

Turn it on in your account settings under **Two-factor authentication**. Your options:
...
App lives in a subfolder of your repo? Set a **source directory** and Nubo builds from there instead of the root. Handy for monorepos.

Set it when you first create a Frame (in the deploy wizard, quick create, or the new-Frame modal) and change it later in **Frame Settings**. Works on managed cloud and on your own agents.
...
Nubo writes back to GitHub too, with commit status checks and a PR comment carrying the preview link. Close the PR and the preview tears itself down. See [Preview deployments](/preview-deployments).
...
<Update label="May 28th, 2026" tags={["Databases", "CLI"]}>
...
You don't have to run your own database anymore. Create a managed **Postgres** from a Project, pick a size, and Nubo hands you a connection string. It shows up alongside your apps as a Frame, so it starts, stops, and appears in your lists like everything else, and its data sticks around between deploys.

Wiring it into an app is one step: point the database at the Frame that needs it and Nubo drops the connection string in as an environment variable (`DATABASE_URL` by default). Your app picks it up on its next deploy.

More engines are on the way. For now it's Postgres, which covers most of what people reach for. See [Databases](/databases).
...
Sign in with `nubo login`, then browse your projects, Frames, databases, volumes, and agents straight from the terminal, either with quick `ls` commands or a full-screen interactive view with live logs and metrics. Add `--json` to anything to pipe it into a script.

This first version is read-only: it's for looking, scripting, and keeping an eye on things, not yet for deploying or editing. Those come next. See the [CLI guide](/cli).
...
Sizes go up to 1 TB. Each volume attaches to one Frame at a time. To move it to another Frame, detach it first. Spin one up from the new **Volumes** page on any Project, or from **Quick actions** in the sidebar.

Billing runs by the hour for every volume you've provisioned, whether or not it's attached. Detaching doesn't stop the meter. Delete the volume to stop it.

### Pick a plan that fits

Nubo now has plans. Every new account starts on a **7-day Trial** on us, so you can build something real before you commit. The Trial runs at trial-level compute (about half of Basic).
...
Install the [Nubo GitHub App](https://github.com/apps/nubo-application) on your repo. Every push to your Frame's tracked branch triggers a build and release.
...
<Update label="February 24th, 2026" tags={["Dashboard", "Design"]}>
