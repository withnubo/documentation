...
The installer sends anonymous telemetry by default so we can spot install failures across platforms. No source code, environment variables, or identifiers are collected. Turn it off with the `--no-telemetry` flag, or by setting `NUBO_TELEMETRY=0` or `DO_NOT_TRACK=1` in the environment before you run the installer.
