...
1. A `project.toml` that sets a web server, a web root, and a port.
...
`BP_PHP_WEB_DIR` is the folder that holds `index.php`. `BPE_PORT` sets the runtime port (and `PORT` env var). The Frame's port setting should match.
...
<Steps>
  ...
  <Step title="Confirm the port">
    Match what `BPE_PORT` is set to (8080 here).
  </Step>
  <Step title="Deploy">
