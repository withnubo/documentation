...
Out of the box, Studio runs on Nubo's default models, so you can start building without configuring anything. You can also bring your own key for Claude, OpenAI, Gemini, or any OpenAI-compatible endpoint, including a local runner like Ollama or LM Studio. Each of the three agents can use a different model, so you might put a stronger model on the Developer and a lighter one on the Project Manager.
...
Every Nubo plan includes a monthly allowance of AI tokens for generation on Nubo's default models: 250K on the free Trial, 2M on Basic, 10M on Silver, and 20M on Platinum. When you reach your allowance, generation pauses and you can either bring your own model key, which bypasses the budget and is billed by your provider rather than Nubo, or upgrade your plan for a higher allowance.

Published apps carry a small "Built with Nubo" badge on the free Trial. Removing the badge and pointing a [custom domain](/custom-domains) at a published app are both included on the paid plans.
