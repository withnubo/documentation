...
You sign in to Nubo with GitHub (there's no separate email and password), and that one sign-in carries across the console and Studio. Adding a second factor means a stolen GitHub session isn't enough to get into your Nubo account: after the GitHub step, we ask for a code from your authenticator app or a tap on your passkey.
...
Enable either one (or both) from **Settings > Security** in the [dashboard](https://console.withnubo.com). (Your name and company live under **Settings > Account**.)
