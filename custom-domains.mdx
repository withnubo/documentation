...
Every Frame gets a Nubo-assigned domain out of the box (something like `your-app-a1b2.nubo.onl`), and PR preview deployments get their own hostname under `*.preview.nubo.onl`. When you're ready to go live you can attach a custom domain (anything like `shop.mystore.com`, `api.mysite.io`, or even a root domain) and Nubo will serve the Frame there with HTTPS.
...
<Note>
  A custom hostname has to be a real domain: at least two labels (`shop.mystore.com`, not `localhost`), ASCII only, no wildcards (`*.mystore.com`), and no underscores. Each hostname attaches to one Frame at a time.
</Note>
...
Until DNS points at that target, `/verify` reports that the hostname does not yet resolve; keep polling as DNS propagates. Once it resolves, Nubo issues a free Let's Encrypt certificate (renewed automatically) and starts serving traffic at your hostname. You can call `/verify` again any time to re-poll.
...
A Frame can have several custom domains attached at once. Mark one as the **primary** domain to make it the canonical hostname for the Frame. Only a domain that's already `active` can be made primary:

```bash
curl -X POST https://shuttle.withnubo.com/v2/projects/<project_id>/spaces/<space_id>/frames/<frame_id>/domains/shop.mystore.com/primary \
  -H "Authorization: Bearer <your-token>"
```

Calling it again on the current primary toggles the flag back off.
