...
A deployment is one build-and-release of a [Frame](/frames). Nubo builds the new version, waits for it to be healthy, and only then cuts traffic over. If anything goes wrong, the previous version keeps serving.
