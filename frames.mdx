...
A Frame is one deployable app - your landing page, API, Discord bot, or anything else you'd normally stand up on its own server. Each Frame gets its own domain, its own deploy history, and its own settings. Frames live inside a [Space](/concepts-and-terms#space), and Spaces live inside a [Project](/concepts-and-terms#project).

Frames come in two kinds: an App Frame (the code you deploy) and a [Database](/databases) Frame (managed storage your apps connect to). Everything below covers App Frames unless noted.
...
The Frame's **Settings** tab groups everything you can change into clear sections (General, Visibility, Source, Build, Runtime, Continuous deployment, Preview deployments, Environment variables, Custom domains, Danger zone). A sticky save bar at the bottom tracks unsaved changes so you always know if something is pending.

You can:

- Rename the Frame.
- Set the Frame's visibility. A Frame is Private by default; set it Public to list it on your organization's [public profile](/profiles). Changing visibility requires an organization admin.
