...
A couple of rules apply to what a volume can attach to:

- **Single writer.** A volume has one writer at a time, so it attaches to a Frame running a single replica. A Frame scaled beyond one replica can't mount it.
- **Same region.** The volume and the Frame have to be in the same region.
...
<Note>
  Mount paths are validated. The path has to be absolute (start with `/`) and can't contain `..`. You also can't mount on a reserved system root or any of its subdirectories: `/`, `/bin`, `/boot`, `/dev`, `/etc`, `/lib`, `/proc`, `/root`, `/run`, `/sbin`, `/sys`, `/usr`, and `/var`.
</Note>
...
You pay by the provisioned size of a managed volume for as long as it exists, whether it's attached or not, and whether the Frame it's attached to is running or stopped. A 50 GiB volume costs half what a 100 GiB one does. Detaching or stopping the Frame does not stop the meter; delete the volume to stop the meter.
